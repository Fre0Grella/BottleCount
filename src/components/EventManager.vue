...
async function createParty() {
  ...
  if (token && syncConfig.value) pushParties(token, syncConfig.value.sheetId, [p]).catch(() => {});
}
...
async function addTicket() {
  ...
  await ensureQR(t);
  const token = getAccessToken();
  if (token && syncConfig.value) pushTickets(token, syncConfig.value.sheetId, [t]).catch(() => {});
}
...
async function syncNow() {
  const token = getAccessToken();
  if (!token || !syncConfig.value) { await connectGoogle(); return; }
  ...
  try {
    ...
    for (const rp of remote.parties) {
      ...
      if (!(await db.parties.get(id))) {
        await db.parties.put({ id, name: rp.name, date: rp.date ?? '', createdAt: rp.createdAt });
      }
    }
    for (const rt of remote.tickets) {
      const id = Number(rt.id); if (!id) continue;
      const local      = await db.tickets.get(id);
      const remoteUsed = rt.used === 'TRUE' || rt.used === 'true';
      if (!local) {
        await db.tickets.put({ id, partyId: Number(rt.partyId), guestName: rt.guestName, used: remoteUsed, usedAt: rt.usedAt || undefined, expiresAt: rt.expiresAt });
...
function stopScanner() {
  scanner?.stop(); scanner?.destroy(); scanner = null; scanning.value = false;
}

async function handleScan(qrString: string) {
  scanner?.stop();
  const payload = await verifyTicket(qrString);
  if (!payload) { setResult(false, '⛔ Forged or invalid ticket', ''); scanner?.start(); return; }
  ...
  if (!ticket) { setResult(false, '❓ Unknown ticket — try syncing first', payload.guestName); scanner?.start(); return; }
  if (ticket.used) { setResult(false, `🔁 Already used at ${ticket.usedAt ? new Date(ticket.usedAt).toLocaleTimeString() : '?'}`, payload.guestName); scanner?.start(); return; }

  const usedAt = new Date().toISOString();
  await db.tickets.update(payload.ticketId, { used: true, usedAt });
  const token = getAccessToken();
  if (token && syncConfig.value) markTicketUsed(token, syncConfig.value.sheetId, payload.ticketId, usedAt).catch(() => {});
...
<template>
  ...
  <div v-else>
    ...
    <div class="em-sync-banner">
      <div v-if="!syncConfig" class="card card-pad em-sync-connect">
        <div class="em-sync-connect__text">
          ...
          <div class="em-sync-connect__sub">Connect Google to share parties &amp; tickets between phones</div>
        </div>
      ...
      <div v-else class="card card-pad em-sync-status">
        <span class="em-sync-status__label"
          :class="{ ok: syncStatus==='ok', err: syncStatus==='error', warn: syncStatus==='syncing' || syncStatus==='connecting' }">
          {{ syncLabel }}
        </span>
        <div class="em-sync-status__actions">
          ...
          <button class="btn btn-ghost btn-sm em-sync-status__disconnect" @click="disconnectGoogle">Disconnect</button>
        </div>
      </div>
    </div>
    ...
    <div class="card card-pad em-party-selector">
        ...
        <div class="em-party-form__menu-section">
          ...
          <div class="em-menu-options">
            <label v-for="opt in [
              ...
              { v:'empty',   e:'✏️',  t:'From scratch', d:'Empty categories' },
            ]" :key="opt.v"
              class="em-menu-option"
              :class="{ 'em-menu-option--active': partyForm.menuTemplate === opt.v }">
              <input type="radio" v-model="partyForm.menuTemplate" :value="opt.v" class="em-menu-option__radio" />
              <div>
                <div class="em-menu-option__title">{{ opt.e }} {{ opt.t }}</div>
                <div class="em-menu-option__sub">{{ opt.d }}</div>
              </div>
            </label>
          </div>
        </div>
      ...
      <div v-for="p in parties" :key="p.id"
        ...
        <div class="em-party-row__info">
          ...
          <div class="em-party-row__date">{{ p.date ? new Date(p.date).toLocaleDateString() : 'No date' }}</div>
        </div>
    ...
    <div v-if="selected">
      <div class="em-tabs">
        <button v-for="tab in [{k:'tickets',l:'🎟 Tickets'},{k:'scanner',l:'📷 Scanner'}]" :key="tab.k"
          class="em-tab" :class="{ 'em-tab--active': activeTab === tab.k }"
      ...
      <div v-show="activeTab==='tickets'">
        ...
        <div class="grid-3 em-stats">
          <div class="card card-pad em-stat"><div class="em-stat__value em-stat__value--green">{{ tickets.length }}</div><div class="em-stat__label">Total</div></div>
          <div class="card card-pad em-stat"><div class="em-stat__value em-stat__value--indigo">{{ usedCount }}</div><div class="em-stat__label">Used</div></div>
          <div class="card card-pad em-stat"><div class="em-stat__value em-stat__value--red">{{ expiredCount }}</div><div class="em-stat__label">Expired</div></div>
        </div>
        ...
        <div v-for="t in tickets" :key="t.id" class="em-ticket-row card">
          <img v-if="qrImages[t.id!]" :src="qrImages[t.id!]" class="em-ticket-row__qr" alt="QR code" />
          <div v-else class="em-ticket-row__qr em-ticket-row__qr--placeholder">…</div>
          <div class="em-ticket-row__info">
            <div class="em-ticket-row__name">{{ t.guestName }}</div>
            <div class="em-ticket-row__expiry">Valid until {{ new Date(t.expiresAt).toLocaleDateString() }}</div>
            <div class="em-ticket-row__badges">
              <span v-if="t.used"                                 class="badge em-badge--used">✓ Used</span>
              <span v-else-if="new Date(t.expiresAt) < new Date()" class="badge em-badge--expired">Expired</span>
      ...
      <div v-show="activeTab==='scanner'">
        <div class="em-scanner-viewport card">
          ...
          <div v-if="lastResult" class="em-scan-result" :class="lastResult.ok ? 'em-scan-result--ok' : 'em-scan-result--err'">
            <div class="em-scan-result__icon">{{ lastResult.ok ? '✅' : '❌' }}</div>
        ...
        <div class="card card-pad em-manual-entry">
          ...
          <div class="em-manual-entry__row">
            <input type="text" id="manual-qr" placeholder="Paste QR string here…"
              @keyup.enter="handleScan(($event.target as HTMLInputElement).value)" />
    ...
    <div v-if="shareTicket" class="em-overlay" @click.self="closeShare">
      <div class="em-share-modal card">
        ...
        <div class="em-share-modal__preview">
          <img v-if="shareImg" :src="shareImg" class="em-share-modal__qr" alt="QR preview" />
          <div class="em-share-modal__guest">{{ shareTicket.guestName }}</div>
          <div class="em-share-modal__party">{{ selected?.name }}</div>
          <div class="em-share-modal__hint">A PNG ticket image will be generated</div>
        </div>
        <div v-if="shareWorking" class="em-share-modal__working">⏳ Generating image…</div>
        <div v-else class="em-share-modal__actions">
          <button class="btn btn-primary btn-full" @click="shareNative">📱 Share via…</button>
          <button class="btn btn-ghost   btn-full em-share-modal__download" @click="downloadImage">🖼️ Download as Image</button>
        </div>
      </div>
    </div>
...
<style scoped>
/* ── Loading ───────────────────────────────────────────── */
.em-loading { text-align: center; padding: 60px; color: var(--muted); }

/* ── Sync banner ───────────────────────────────────────── */
.em-sync-banner { margin-bottom: 16px; }
.em-sync-connect { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; border-style: dashed; }
.em-sync-connect__title { font-weight: 700; font-size: .9rem; }
.em-sync-connect__sub   { font-size: .78rem; color: var(--muted); margin-top: 2px; }
.em-sync-status         { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
.em-sync-status__label  { font-size: .85rem; }
.em-sync-status__actions { display: flex; gap: 8px; }
.em-sync-status__disconnect { font-size: .75rem; color: var(--muted); }

/* ── Shared section header ──────────────────────────────── */
.em-section-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.em-section-header .section-label { margin: 0; }

/* ── Party selector ────────────────────────────────────── */
.em-party-selector { margin-bottom: 16px; }
.em-party-form { background: var(--surface2); border: 1px solid var(--border2); border-radius: 10px; padding: 12px; margin-bottom: 12px; }
.em-party-form__fields       { margin-bottom: 10px; }
.em-party-form__menu-section { margin-bottom: 12px; }
.em-menu-options  { display: flex; gap: 10px; flex-wrap: wrap; }
.em-menu-option   { display: flex; align-items: center; gap: 8px; cursor: pointer; flex: 1; min-width: 140px;
                    background: var(--surface); border: 2px solid var(--border2); border-radius: 10px;
                    padding: 10px 14px; transition: border-color .15s; }
.em-menu-option--active   { border-color: var(--indigo); }
.em-menu-option__radio    { width: auto; display: inline; }
.em-menu-option__title    { font-weight: 700; font-size: .9rem; }
.em-menu-option__sub      { font-size: .75rem; color: var(--muted); }
.em-party-row     { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 10px;
                    cursor: pointer; margin-bottom: 4px; border: 1px solid transparent;
                    transition: background .15s, border-color .15s; }
.em-party-row:hover       { background: var(--border); }
.em-party-row--active     { background: var(--border); border-color: var(--indigo); }
.em-party-row__info       { flex: 1; min-width: 0; }
.em-party-row__name       { font-weight: 700; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.em-party-row__date       { font-size: .75rem; color: var(--muted); }

/* ── Tabs ──────────────────────────────────────────────── */
.em-tabs    { display: flex; gap: 4px; margin-bottom: 16px; background: var(--surface);
              border: 1px solid var(--border); border-radius: 12px; padding: 4px; }
.em-tab     { flex: 1; padding: 8px; border-radius: 9px; font-weight: 700; font-size: .9rem;
              border: none; cursor: pointer; background: transparent; color: var(--muted);
              transition: background .15s, color .15s; }
.em-tab--active { background: var(--indigo); color: #fff; }

/* ── Tickets tab ─────────────────────────────────────────── */
.em-tickets-header          { flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }
.em-tickets-header__title   { font-size: 1rem; font-weight: 700; margin: 0; }
.em-tickets-header__actions { display: flex; gap: 8px; flex-wrap: wrap; }
.em-stats  { margin-bottom: 16px; }
.em-stat   { text-align: center; }
.em-stat__value         { font-size: 1.5rem; font-weight: 900; }
.em-stat__label         { font-size: .75rem; color: var(--muted); }
.em-stat__value--green  { color: #4ade80; }
.em-stat__value--indigo { color: #818cf8; }
.em-stat__value--red    { color: #f87171; }
.em-guest-form          { margin-bottom: 12px; }
.em-guest-form__fields  { margin-bottom: 8px; }

/* ── Ticket rows ───────────────────────────────────────── */
.em-ticket-row          { display: flex; align-items: flex-start; gap: 12px; padding: 12px; margin-bottom: 8px; }
.em-ticket-row__qr      { width: 80px; height: 80px; border-radius: 6px; flex-shrink: 0; background: #fff; }
.em-ticket-row__qr--placeholder { background: var(--surface2); display: flex; align-items: center;
                                   justify-content: center; font-size: .7rem; color: var(--muted); }
.em-ticket-row__info    { flex: 1; min-width: 0; }
.em-ticket-row__name    { font-weight: 700; font-size: 1rem; margin-bottom: 2px; }
.em-ticket-row__expiry  { font-size: .75rem; color: var(--muted); margin-bottom: 6px; }
.em-ticket-row__badges  { display: flex; gap: 6px; align-items: center; flex-wrap: wrap; }
.em-ticket-row__id      { font-size: .7rem; color: var(--muted); }
.em-ticket-row__actions { display: flex; flex-direction: column; gap: 4px; flex-shrink: 0; }
.em-badge--used    { background: #14532d; color: #86efac; }
.em-badge--expired { background: #7c2d12; color: #f87171; }
.em-badge--valid   { background: #0c4a6e; color: #38bdf8; }

/* ── Scanner ───────────────────────────────────────────── */
.em-scanner-viewport { overflow: hidden; margin-bottom: 16px; position: relative; }
.em-scanner-video    { width: 100%; display: block; aspect-ratio: 4/3; background: #000; object-fit: cover; }
.em-scanner-controls { margin-bottom: 16px; }
.em-scan-result          { position: absolute; inset: 0; display: flex; flex-direction: column;
                           align-items: center; justify-content: center; }
.em-scan-result--ok  { background: rgba(34,197,94,.85); }
.em-scan-result--err { background: rgba(239,68,68,.85); }
.em-scan-result__icon    { font-size: 3rem; }
.em-scan-result__message { font-weight: 800; font-size: 1.1rem; color: #fff; margin-top: 8px; text-align: center; padding: 0 16px; }

/* ── Manual entry ──────────────────────────────────────── */
.em-manual-entry      { margin-bottom: 16px; }
.em-manual-entry__row { display: flex; gap: 8px; }

/* ── Scan history ──────────────────────────────────────── */
.em-scan-history__header { padding: 12px 16px; font-weight: 700; font-size: .9rem; border-bottom: 1px solid var(--border); }
.em-scan-history__icon   { font-size: 1.1rem; }
.em-scan-history__name   { flex: 1; font-weight: 500; }
.em-scan-history__time   { font-size: .75rem; color: var(--muted); }

/* ── Empty state ───────────────────────────────────────── */
.em-empty { color: var(--muted); font-size: .875rem; text-align: center; padding: 40px; }

/* ── Share overlay ─────────────────────────────────────── */
.em-overlay              { position: fixed; inset: 0; background: rgba(0,0,0,.75); z-index: 200;
                           display: flex; align-items: center; justify-content: center; padding: 16px; }
.em-share-modal          { padding: 24px; width: 100%; max-width: 380px; }
.em-share-modal__header  { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
.em-share-modal__title   { font-weight: 800; font-size: 1rem; }
.em-share-modal__preview { text-align: center; margin-bottom: 16px; }
.em-share-modal__qr      { width: 120px; height: 120px; border-radius: 8px; border: 2px solid var(--border2); background: #fff; }
.em-share-modal__guest   { font-weight: 700; margin-top: 8px; }
.em-share-modal__party   { font-size: .8rem; color: var(--muted); }
.em-share-modal__hint    { font-size: .72rem; color: var(--muted); margin-top: 6px; }
.em-share-modal__working { text-align: center; padding: 20px; color: var(--muted); }
.em-share-modal__actions { display: flex; flex-direction: column; gap: 10px; }
.em-share-modal__download { border: 1px solid var(--border2); }
</style>

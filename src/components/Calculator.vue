...
import type {
  Settings, Catalog, CalculationResult, Ingredient, Cocktail,
  Party, PartyMenu, PartySettings,
} from '../lib/types';
...
// ─────────────────────────────────────────────────────────────────────────────
const globalSettings = ref<Settings>(JSON.parse(JSON.stringify(defaultSettings)) as Settings);
...
function syncActiveConfig() {
  ...
  const pm: PartyMenu      = p.partyMenu    ?? JSON.parse(JSON.stringify(defaultSettings.menu));
  settings.value = {
...
const alcoholLabel = computed(() => ALCOHOL_LEVELS[settings.value.alcohol_ml_per_person] ?? '');

const isGlobal   = computed(() => selectedPartyId.value === null);
const modeLabel  = computed(() => isGlobal.value ? 'Global defaults' : selectedParty.value?.name ?? '');
const saveLabel  = computed(() => {
  if (activeTab.value === 'catalog') return null;
  if (activeTab.value === 'settings') return isGlobal.value ? '💾 Save global settings' : `💾 Save ${modeLabel.value}`;
  return isGlobal.value ? '💾 Save global menu' : `💾 Save ${modeLabel.value} menu`;
});
...
async function handleSave() {
  ...
  try {
    if (isGlobal.value) {
      ...
      if (idx >= 0) {
        parties.value[idx] = { ...parties.value[idx], partySettings: ps, partyMenu: pm };
      }
    }
...
function spiritsSum(cat: string) {
  return Object.values(settings.value.menu[cat]?.spirits ?? {}).reduce((s, v) => s + v.pct, 0);
}
function drinksSum(cat: string, sp: string) {
  return Object.values(settings.value.menu[cat]?.spirits?.[sp]?.drinks ?? {}).reduce((s, v) => s + v, 0);
}
function macroSum() { return Object.values(settings.value.menu).reduce((s, v) => s + v.macro_pct, 0); }
...
function addSpirit(cat: string, name: string) {
  ...
  settings.value.menu[cat].spirits[name] = SIMPLE.has(cat) ? { pct: 0 } : { pct: 0, drinks: {} };
}
function removeSpirit(cat: string, name: string) { delete settings.value.menu[cat].spirits[name]; }
...
function cocktailsFor(sn: string) {
  return Object.entries(catalog.value.cocktails).filter(([, c]) => c.main_spirit === sn).map(([n]) => n);
}
function spiritsByType(t: 'spirit' | 'beer' | 'wine') {
  return Object.entries(catalog.value.ingredients).filter(([, i]) => i.type === t).map(([n]) => n);
}
...
async function addIngredient() {
  ...
  const newIng: Ingredient = {
    type: ingForm.value.type as Ingredient['type'], abv: ingForm.value.abv,
    volume_ml: ingForm.value.volume_ml > 0 ? ingForm.value.volume_ml : undefined,
    unit: ingForm.value.unit, price_min: ingForm.value.price_min, price_max: ingForm.value.price_max,
  };
  personal[name] = newIng; await setKey('personal_ingredients', personal);
...
async function deleteIngredient(name: string) {
  ...
  if (def[name]) {
    ...
    if (!h.includes(name)) h.push(name); await setKey('hidden_ingredients', h);
  } else {
    const p = await getKey<Record<string, Ingredient>>('personal_ingredients', {});
    delete p[name]; await setKey('personal_ingredients', p);
  }
  delete catalog.value.ingredients[name]; delete priceEdits.value[name];
}
async function savePrice(name: string) {
  ...
  const def = (defaultCatalog as Catalog).ingredients;
  if (def[name]) {
    const ov = await getKey<Record<string, Partial<Ingredient>>>('price_overrides', {});
    ov[name] = { price_min: min, price_max: max }; await setKey('price_overrides', ov);
  } else {
    const p = await getKey<Record<string, Ingredient>>('personal_ingredients', {});
    if (p[name]) { p[name].price_min = min; p[name].price_max = max; } await setKey('personal_ingredients', p);
  }
  catalog.value.ingredients[name].price_min = min; catalog.value.ingredients[name].price_max = max;
...
async function addCocktail() {
  ...
  if (!Object.keys(ckRecipe.value).length) { alert('Add at least one ingredient'); return; }
  const personal = await getKey<Record<string, Cocktail>>('personal_cocktails', {});
  const newCk: Cocktail = {
    main_spirit: ckForm.value.main_spirit, category: ckForm.value.category,
    recipe: Object.fromEntries(Object.entries(ckRecipe.value).map(([k, v]) => [k, { quantity: v.quantity }])),
  };
  personal[name] = newCk; await setKey('personal_cocktails', personal);
  catalog.value.cocktails[name] = newCk;
  ckForm.value = { name: '', main_spirit: spiritIngredients.value[0] ?? '', category: 'Spirits' };
  ckRecipe.value = {}; showCkForm.value = false;
}
async function deleteCocktail(name: string) {
  ...
  if (def[name]) {
    ...
    if (!h.includes(name)) h.push(name); await setKey('hidden_cocktails', h);
  } else {
    const p = await getKey<Record<string, Cocktail>>('personal_cocktails', {});
    delete p[name]; await setKey('personal_cocktails', p);
  }
...
function exportTxt() {
  ...
  const label = isGlobal.value ? 'GLOBAL' : (selectedParty.value?.name?.toUpperCase() ?? '');
  const lines = [
    '='.repeat(62), `  PARTY BUDGET${label ? ' — ' + label : ''} — ${s.guests} GUESTS`, '='.repeat(62),
    `  Ticket price : €${s.ticket_price}`, `  Venue cost   : €${s.venue_cost}`,
    `  Equipment    : €${s.equipment_cost}`, `  Revenue      : €${r.revenue}`,
    `  Profit       : €${r.profit_min} – €${r.profit_max}`, `  Break-even   : ${r.break_even} guests`,
    '', '='.repeat(62), '  SHOPPING LIST', '='.repeat(62),
...
<template>
  ...
  <div v-else>
    ...
    <div class="card card-pad" style="margin-bottom:16px;">
      <div class="section-label" style="margin-bottom:8px;">Calculating for</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        ...
        <button v-for="p in parties" :key="p.id"
          class="btn btn-sm"
          :class="selectedPartyId===p.id ? 'btn-primary' : 'btn-ghost'"
          @click="selectPartyId(p.id ?? null)">
          🎉 {{ p.name }}
        </button>
        <span v-if="!parties.length" style="font-size:.8rem;color:var(--muted);">
          Create parties in the Tickets section to get per-party configs.
        </span>
      </div>
      <!-- Context label -->
      <div style="margin-top:10px;font-size:.8rem;color:var(--muted);">
        <span v-if="isGlobal">Editing <strong style="color:var(--text)">global defaults</strong> — shared baseline for all parties</span>
        <span v-else>Editing <strong style="color:#818cf8">{{ selectedParty?.name }}</strong> — independent settings &amp; drink menu</span>
      </div>
    </div>
    ...
    <template v-if="activeTab==='settings'">
      <div class="card card-pad">
        <div class="section-label">{{ isGlobal ? 'Global Settings' : selectedParty?.name + ' Settings' }}</div>
        <div class="grid-2" style="margin-bottom:14px;">
          <div><label>Guests</label><input type="number" v-model.number="settings.guests" min="1" /></div>
          <div><label>Ticket €</label><input type="number" v-model.number="settings.ticket_price" min="0" step="0.5" /></div>
          <div><label>Venue €</label><input type="number" v-model.number="settings.venue_cost" min="0" /></div>
          <div><label>Equipment €</label><input type="number" v-model.number="settings.equipment_cost" min="0" /></div>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;font-size:.7rem;color:var(--indigo);margin-bottom:4px;">
      ...
      <div v-if="result" class="card">
        ...
        <div style="display:flex;gap:8px;padding:6px 12px;background:#252538;font-size:.7rem;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);border-bottom:1px solid var(--border2);">
          <span style="flex:1;">Item</span><span style="width:52px;text-align:right;">Qty</span>
          <span style="width:52px;text-align:right;color:#4ade80;">Min€</span><span style="width:52px;text-align:right;color:#f87171;">Max€</span>
        </div>
    ...
    <template v-if="activeTab==='menu'">
      <!-- Context banner -->
      <div v-if="!isGlobal" style="background:#1e1b4b;border:1px solid #4338ca;border-radius:10px;padding:10px 14px;margin-bottom:12px;font-size:.85rem;">
      ...
      <div class="card card-pad">
        ...
        <div style="font-size:.85rem;">Total: <span :class="pctOk(macroSum())?'ok':'err'">{{ (macroSum()*100).toFixed(1) }}%</span></div>
      </div>
      ...
      <div class="card" v-for="(catData,cat) in settings.menu" :key="cat" style="margin-bottom:12px;">
        ...
        <div style="padding:12px 16px;">
          <template v-if="SIMPLE.has(cat as string)">
            <p style="font-size:.75rem;color:var(--muted);margin-bottom:10px;">Set share per variety (must = 100%)</p>
    ...
    <template v-if="activeTab==='catalog'">
      ...
      <template v-if="catSection==='ingredients'">
        <button class="btn btn-primary btn-sm btn-full" style="margin-bottom:12px;" @click="showIngForm=!showIngForm">{{ showIngForm?'✕ Cancel':'+ Add ingredient' }}</button>
        <div v-if="showIngForm" class="card card-pad" style="margin-bottom:12px;">
          <div class="grid-2" style="margin-bottom:8px;">
            ...
            <div><label>Type</label>
              <select v-model="ingForm.type"><option>spirit</option><option>beer</option><option>wine</option><option>mixer</option><option>snack</option><option>extra</option></select>
            </div>
        ...
        <div v-for="(ing,name) in catalog.ingredients" :key="name"
          ...
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;" v-if="priceEdits[name as string]">
            ...
            <button class="btn btn-success btn-sm" @click="savePrice(name as string)">{{ savingPrice===name?'✅':'💾' }}</button>
          </div>
        </div>
      </template>
      ...
      <template v-if="catSection==='cocktails'">
        <button class="btn btn-primary btn-sm btn-full" style="margin-bottom:12px;" @click="showCkForm=!showCkForm">{{ showCkForm?'✕ Cancel':'+ Add cocktail' }}</button>
        <div v-if="showCkForm" class="card card-pad" style="margin-bottom:12px;">
          <div class="grid-2" style="margin-bottom:8px;">
            ...
            <div><label>Main spirit</label>
              <select v-model="ckForm.main_spirit"><option v-for="n in spiritIngredients" :key="n" :value="n">{{ n }}</option></select>
            </div>
          </div>
          ...
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;">
            <select v-model="ckIngSel" style="flex:1;min-width:0;"><option v-for="n in allIngredientNames" :key="n" :value="n">{{ n }}</option></select>
    ...
    <div v-if="saveLabel" class="save-bar">
      <div class="save-bar-inner">
        <span v-if="errors.length && activeTab==='menu'" class="err" style="font-size:.82rem;flex:1;">
          ⚠️ {{ errors.length }} error{{ errors.length > 1 ? 's' : '' }}
        </span>
        <span v-else style="flex:1;font-size:.82rem;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
          {{ isGlobal ? '🌐 Global' : '🎉 ' + selectedParty?.name }}
          · {{ settings.guests }} guests · €{{ settings.ticket_price }} ticket
        </span>
        <button class="btn btn-success" @click="handleSave" :disabled="saving" style="min-width:150px;flex-shrink:0;">

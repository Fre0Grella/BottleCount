...
const CLIENT_ID =
  '402376254532-ji43fivhq1ksni3pahe1hrsblvsbsm7l.apps.googleusercontent.com';
const SCOPES =
...
export async function connectSheet(token: string): Promise<string> {
  ...
  await batchUpdate(token, sheetId, [
    ...
    {
      range: 'tickets!A1',
      values: [['id', 'partyId', 'guestName', 'used', 'usedAt', 'expiresAt']],
    },
...
async function validateHeaders(token: string, sheetId: string) {
  for (const tab of ['parties', 'tickets'] as const) {
    ...
    const headers: string[] = (res.values?.[0] ?? []).map((h: string) =>
      h.trim().toLowerCase(),
    );
 ...
 */
export async function syncHmacKey(
  token: string,
  sheetId: string,
  localJwk: JsonWebKey,
): Promise<JsonWebKey> {
  ...
  if (existing?.[1]) {
    ...
    try {
      return JSON.parse(existing[1]) as JsonWebKey;
    } catch {
  ...
  await appendRows(token, sheetId, 'config', [
    ['hmac_key', JSON.stringify(localJwk)],
  ]);
  return localJwk;
}
...
export async function pullAll(token: string, sheetId: string) {
  const [partiesRes, ticketsRes] = await Promise.all([
    gapi(
      token,
      'GET',
      `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/parties!A1:Z`,
    ),
    gapi(
      token,
      'GET',
      `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/tickets!A1:Z`,
    ),
  ]);
  return {
...
export async function pushParties(
  token: string,
  sheetId: string,
  parties: any[],
) {
...
async function gapi(token: string, method: string, url: string, body?: any) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      (err as any)?.error?.message ?? `Google API error ${res.status}`,
    );
  }
  return res.json();
}
